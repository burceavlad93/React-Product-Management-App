...
function SizeInput({ value, onChange, isValid, isMissing }) {
  ...
  const [errorMessage, setErrorMessage] = useState("Size field is required.");
  ...
  function handleBlur() {
    ...
    setErrorMessage(value.trim() === "" ? "Size field is required." : "Please provide a valid number.");
  ...
  return (
    <div>
      <div className={`input-group mb-3 ${touched && (error ? "has-error" : "has-success")}`}>
        <label htmlFor="size" className="input-group-text" id="inputGroup-sizing-default">
          Size (MB)
        </label>
        <input
          type="number"
